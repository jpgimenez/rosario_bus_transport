"""Config flow to configure the RosarioBus integration."""
...
def _get_agency_tags(client: RosarioBusClient) -> dict[str, str]:
#    return {a["tag"]: a["title"] for a in client.get_agency_list()["agency"]}
    return {"MOVI": "MOVI"}

def _get_route_tags(
    client: RosarioBusClient, agency_tag: str, stop_tag: str
) -> dict[str, str]:
#    return {a["tag"]: a["title"] for a in client.get_route_list(agency_tag)["route"]}
    return {"110": "110"}

def _get_stop_tags(
    client: RosarioBusClient, agency_tag: str
) -> dict[str, str]:
...
#            tags[tag] = f"{title} ({stop_directions.get(tag, tag)})"
    ...
    tags = {
        "2997": "José Ingeniero y Leguizamón"
    }
...
class RosarioBusFlowHandler(ConfigFlow, domain=DOMAIN):
    """Handle RosarioBus configuration."""
    ...
    def __init__(self):
        """Initialize RosarioBus config flow."""
        self.data: dict[str, str] = {
            CONF_AGENCY: DEFAULT_AGENCY  # by now only support MOVI
        }
        self._client = RosarioBusClient(output_format="json")
    ...
    async def async_step_stop(
        ...
        self._stop_tags = await self.hass.async_add_executor_job(
            _get_stop_tags,
            self._client,
            self.data.get(CONF_AGENCY, DEFAULT_AGENCY),
        )
    ...
    async def async_step_route(
        ...
        if user_input is not None:
            ...
            agency_tag = self.data.get(CONF_AGENCY, DEFAULT_AGENCY)
            ...
            return self.async_create_entry(
                title=f"{agency_name} {stop_name} {route_name}",
                data=self.data,
            )

        self._route_tags = await self.hass.async_add_executor_job(
            _get_route_tags,
            self._client, self.data[CONF_AGENCY], self.data[CONF_STOP]
        )
